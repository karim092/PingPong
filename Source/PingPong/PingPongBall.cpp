...
void APingPongBall::Server_Move_Implementation(float DeltaTime)
{
	...
	if(!SetActorLocation(newLoc, true, &hitResult))
	{
		...
		DrawDebugDirectionalArrow(GetWorld(), newLoc + moveVector * 300, newLoc, 30, FColor::Yellow, true, 3, 0, 3);
		...
		newTargetMove.Z = height;
		DrawDebugDirectionalArrow(GetWorld(), newLoc, newTargetMove, 30, FColor::Yellow, true, 3, 0, 3);

		//SetActorLocation(currLoc);
		SetActorLocation(FVector(resetPosition.X, resetPosition.Y, height));
